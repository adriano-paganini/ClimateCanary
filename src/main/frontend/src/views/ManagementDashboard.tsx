...
/** Groups measurements into daily buckets and returns average per day. */
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                // Single call to get all active violations and all departments
                const [allDepts, allViolations] = await Promise.all([
                    DepartmentService.getAll(),
                    ViolationService.getAll({ violationStatus: ViolationStatusEnum.ACTIVE }),
                ]);

                // Load rooms + measurements for every department in parallel
                const results = await Promise.all(
                    allDepts.map(async (dept): Promise<DeptData> => {
                        if (!dept.id) return { dept, rooms: [], violations: [], trends: {} };
                        ...
                        // Collect all measurements for this dept across all rooms
                        const allMeasurements: MeasurementDTO[] = [];
                        await Promise.all(
                            rooms.map(r =>
                                r.id
                                    ? MeasurementService.getAll({ roomId: r.id, from }).then(ms =>
                        ...
                        // Build per-metric daily trend (aggregated across all rooms)
        ...
        <div>
            ...
            <div style={{ padding: '1.5rem 2rem 0' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
                    {[
                        { icon: 'pi-sitemap',             label: 'Departments',       value: deptData.length,  color: '#0369a1' },
                        { icon: 'pi-building',            label: 'Rooms',             value: totalRooms,       color: '#374151' },
                        { icon: 'pi-exclamation-triangle', label: 'Active Violations', value: totalViolations, color: totalViolations > 0 ? '#ef4444' : '#22c55e' },
                    ].map(s => (
                        <div key={s.label} style={{
                            ...
                            <div>
                                <div style={{ fontWeight: 700, fontSize: '1.15rem', color: '#111827', lineHeight: 1 }}>
                                    {s.value}
                                </div>
                        ...
                        >
                            ...
                            <div style={{ padding: '1.25rem' }}>
                                ...
                                <div style={{
                                    ...
                                    {violCount > 0 ? (
                                        <span style={{ display: 'flex', alignItems: 'center', gap: '5px', color: statusColor }}>
                                            <i className="pi pi-exclamation-triangle" style={{ fontSize: '1rem' }} />
                                            <Badge value={violCount} severity="danger" />
                                        </span>
                                    ) : (
                                        <i className="pi pi-check-circle" style={{ color: '#22c55e', fontSize: '1.15rem' }} />
                                    )}
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                                    ...
                                    <span style={{
                                        ...
                                        {violCount === 0 ? 'All clear' : `${violCount} violation${violCount > 1 ? 's' : ''}`}
                                    </span>
                                </div>
                                ...
                                {rooms.length > 0 && (
                                    <div style={{ marginBottom: '1.25rem' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                                            {rooms.map(r => {
                                                const rViol = violations.filter(v => v.roomId === r.id).length;
                                                return (
                                                    <div key={r.id} style={{
                                                        display: 'flex',
                                                        justifyContent: 'space-between',
                                                        ...
                                                        <div
                                                            title={rViol === 0 ? 'No violations' : `${rViol} violation(s)`}
                                                            style={{
                                ...
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        {period === 'week' ? 'Last 7 Days' : 'Last 30 Days'} — Avg. per Day
                                    </div>
                                    {METRICS.map(({ key, label, unit, color }) =>
                                        trends[key] && trends[key]!.length > 0 ? (
                                            ...
                                            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                <span style={{ fontSize: '0.7rem', color: '#9ca3af', width: '80px' }}>{label}</span>
                                                <span style={{ fontSize: '0.8rem', color: '#d1d5db', fontStyle: 'italic' }}>No data</span>
                                            </div>
                                        ),
                                    )}
                                </div>
                                ...
                                <div style={{
                                    ...
                                    marginTop: '0.75rem', color: '#9ca3af', fontSize: '0.8rem',
                                    gap: '0.4rem', alignItems: 'center',
                                }}>
                                    <span>View department</span>
                                    <i className="pi pi-arrow-circle-right" style={{ fontSize: '0.8rem' }} />
                                </div>
                            </div>
                        </div>
                    );
