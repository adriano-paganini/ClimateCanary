...
INSERT INTO USERX_USERX_ROLE (USERX_ID, ROLES)
VALUES ((SELECT ID FROM USERX WHERE USERNAME = 'admin'), 'BUILDING_ADMIN');
...
-- Large room-admin test dataset:
-- 30 days at 5 minute intervals for each metric in Room 1.
-- This mirrors the heavy query window used by the room admin room-view.
INSERT INTO MEASUREMENT (TIMESTAMP, MEASUREMENT, METRIC, ROOM_ID, SENSORSTATION_ID)
SELECT DATEADD('MINUTE', X * 5, TIMESTAMP '2026-04-24 00:00:00'),
       20 + MOD(X, 96) / 16.0,
       'TEMPERATURE',
       (SELECT ID FROM ROOMS WHERE NAME = 'Room 1'),
       (SELECT ID FROM SENSORSTATIONS WHERE NAME = 'Station A')
FROM SYSTEM_RANGE(0, 8639);
