...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item System prüft die aktuelle Belegung des Raums anhand der eingetragenen Abwesenheiten.
	...
	\begin{enumerate}
		...
		\item System zeigt Hinweis: \glqq Keine Daten verfügbar -- Datenschutzbedingungen nicht erfüllt.\grqq
...
\begin{itemize}
	...
	\item \textbf{Vorbedingung}: Grenzwerte sind für den betroffenen Raum konfiguriert. Sensorstation ist aktiv und mit dem Raspberry Pi verbunden.
	\item \textbf{Normalablauf}:
	\begin{enumerate}
		\item Raspberry Pi empfängt Messdaten von der Sensorstation via BLE.
		...
		\item Bei bestätigter Grenzwertüberschreitung: Raspberry Pi setzt Warnstatus und übermittelt diesen an die Sensorstation (via BLE) und die Webapp (via REST POST).
	...
	\begin{enumerate}
		...
		\item LED wechselt auf Grün, Warnmeldung in der Webapp verschwindet. Vergangene Verletzung bleibt historisch gespeichert (\texttt{violationStatus = RESOLVED}).
	\end{enumerate}
	\item \textbf{Designentscheidung}: Eine Grenzwertwarnung wird erst ausgelöst, wenn ein Grenzwert über einen konfigurierbaren Zeitraum (z.B. 3 aufeinanderfolgende Messungen) überschritten wird. Dadurch werden kurzfristige Ereignisse (Fenster kurz öffnen, Atemstoß auf Sensor) herausgefiltert. Die Anzahl der nötigen Messungen ist konfigurierbar und in der SQLite-Datenbank des Raspberry Pi hinterlegt.
\end{itemize}

\subsection{UC-05: Grenzwerte konfigurieren}
\begin{itemize}
	\item \textbf{Akteur}: Hausverwalter
	\item \textbf{Vorbedingung}: Nutzer:in ist als Hausverwalter eingeloggt. Raum und Sensorstation sind im System angelegt.
	\item \textbf{Normalablauf}:
	\begin{enumerate}
		\item Hausverwalter öffnet die Konfigurationsansicht eines Raums.
		...
		\item Der zuständige Raspberry Pi holt sich die aktualisierten Grenzwerte beim nächsten regulären Polling-Intervall selbstständig per GET-Request ab.
		\item Raspberry Pi speichert die neuen Grenzwerte lokal in der SQLite-Datenbank und wendet sie ab der nächsten Messung an.
	\end{enumerate}
	\item \textbf{Designentscheidung}: Der Raspberry Pi fragt Konfigurationsänderungen aktiv per Polling ab -- er betreibt keinen eigenen REST-Server. Das vereinfacht die Implementierung am Edge-Device. Der Nachteil (leichte Verzögerung bei Konfigurationsänderungen) ist für einen Prototyp akzeptabel.
\end{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item Wenn ja: Das Backend setzt ein Flag im Raum-Datensatz (\texttt{belowMinOccupancy = true}). Der Raspberry Pi erkennt dies beim nächsten Polling und speichert ab diesem Zeitpunkt keine Messdaten mehr persistent.
...
\begin{itemize}
	\item \textbf{Akteur}: Systemadministrator
	\item \textbf{Vorbedingung}: Nutzer:in ist als Systemadministrator eingeloggt. Gebäudestruktur (Gebäude, Abteilungen, Räume) ist bereits angelegt.
	\item \textbf{Normalablauf}:
	\begin{enumerate}
		\item Systemadministrator legt ein neues Raspberry Pi-Objekt an. System generiert automatisch eine eindeutige ID.
		\item Systemadministrator ordnet den Raspberry Pi einem Raum zu.
		\item Systemadministrator legt eine neue Sensorstation an. System generiert automatisch eine eindeutige ID.
		\item Systemadministrator ordnet die Sensorstation dem Raspberry Pi zu.
		\item Die generierten IDs werden für die physische Konfiguration der Geräte verwendet (hardcoded auf Arduino, \texttt{conf.yaml} auf Raspberry Pi).
	\end{enumerate}
\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{images/climate-canary-webapp-backend}
	\caption{Klassendiagramm V1 -- fachliches Modell der Webapp-Backend-Daten}
\end{figure}

\textit{Das vollständige Klassendiagramm befindet sich als Draw.io Datei im GitLab Wiki
(Datei: \texttt{images/climate-canary-webapp-backend.png}).}
...
\begin{itemize}
	...
	\item \textbf{User} -- Repräsentiert einen Systembenutzer. Attribute: id, firstName, lastName, email, passwordHash, role, departmentIds, roomId. Assoziationen: gehört einer oder mehreren Abteilungen an, ist einem Büroraum zugeordnet, hat Abwesenheiten. \textit{Designentscheidung: Ein User kann mehreren Abteilungen angehören, um z.B. Management- oder Gebäudeadministrationsrollen zu ermöglichen, die Zugriff auf mehrere Abteilungen benötigen.}
	...
	\item \textbf{Building} -- Repräsentiert ein Gebäude. Attribute: id, name, addressId. Assoziationen: hat eine Adresse, wird von mehreren Abteilungen referenziert.
	\item \textbf{Address} -- Repräsentiert eine Adresse. Attribute: id, country, zipCode, city, street, houseNumber, extra.
	...
	\item \textbf{Room} -- Repräsentiert einen Raum. Attribute: id, name, roomType, departmentId, minOccupancy (default: 5 für Büroräume), raspberryPiId. Assoziationen: hat einen Raspberry Pi, wird von Grenzwerten und Grenzwertüberschreitungen referenziert.
	\item \textbf{RaspberryPi} -- Repräsentiert einen Raspberry Pi. Attribute: id (generiert), roomId, ipAddress, deviceStatus, sensorStationIds. Assoziationen: hat Sensorstationen; Grenzwerte werden über die Raumzuweisung verwaltet.
	\item \textbf{SensorStation} -- Repräsentiert eine Sensorstation (Arduino). Attribute: id (generiert, hardcoded auf Arduino), raspberryPiId, deviceStatus. Assoziationen: gehört zu einem Raspberry Pi, wird von Messungen referenziert.
	\item \textbf{Measurement} -- Repräsentiert einen einzelnen Messdatenpunkt. Attribute: id, sensorStationId, timestamp, temperature, humidity, airQuality. Wird \emph{nur} bei erfüllter Datenschutzbedingung (Mindestbelegung) persistent gespeichert.
	\item \textbf{Threshold} -- Repräsentiert Grenzwerte für einen Raum pro Messgröße. Attribute: id, roomId, metric, upperBound, lowerBound (optional). \textit{Designentscheidung: Ein oberer Grenzwert ist immer verpflichtend; der untere Grenzwert ist optional und muss im Konzept begründet werden, wenn er weggelassen wird.}
	\item \textbf{ThresholdViolation} -- Repräsentiert eine aktive oder historische Grenzwertverletzung. Attribute: id, roomId, thresholdId, startTime, endTime (null solange aktiv), metric, value, violationStatus, measurementIds. \textit{Designentscheidung: Mehrere Measurements müssen herangezogen werden, um kurze Extremwerte zu filtern. Eine Verletzung wird erst bestätigt, wenn z.B. 3 aufeinanderfolgende Messungen den Grenzwert überschreiten.}
	\item \textbf{ClimateHint} -- Repräsentiert einen Raumklima-Tipp. Attribute: id, metric, hintText. Wird bei Grenzwertverletzungen auf dem LCD-Display der Sensorstation und in der Webapp angezeigt. Assoziationen: wird von einem oder mehreren Thresholds referenziert.
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{RaspberryPi} $1$ -- $*$ \texttt{SensorStation}: Ein Raspberry Pi kann mehrere Sensorstationen verwalten (im Prototyp: eine pro Raum).
	\item \texttt{SensorStation} $1$ -- $*$ \texttt{Measurement}: Eine Sensorstation produziert viele Messdaten.
	\item \texttt{Room} $1$ -- $*$ \texttt{Threshold}: Ein Raum hat Grenzwerte pro Messgröße (max. 3: Temperatur, Luftfeuchtigkeit, Luftqualität).
...
\textit{Die vollständigen Papierprototypen der GUI befinden sich im Repository unter \\
 \texttt{docs/Softwarekonzept\_V1/images/gui\_prototype}. Folgend werden die Kernansichten textuell beschrieben, weiters werden einzelne Prototypen direkt im Dokument eingebunden.}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.78\textwidth]{images/gui_prototype/Paperprototype_login.png}
	\caption{Papierprototyp der Login-Ansicht}
\end{figure}
...
\begin{itemize}
	...
	\item Prominente Warnanzeige wenn aktive Grenzwertverletzung im eigenen Büro vorhanden, inkl. zugehörigem Raumklima-Tipp.
	\item Navigation zu: Verlaufsansicht, Abwesenheitsverwaltung.
\end{itemize}

\begin{figure}[p]
	\centering
	\includegraphics[width=0.88\textwidth]{images/gui_prototype/Paperprototype_MA_Dashboard.png}
	\caption{Papierprototyp des Dashboards für Mitarbeiter:innen mit Verlaufsansicht und Abwesenheitsverwaltung}
\end{figure}

\subsection{Verlaufsansicht (Raumdetails)}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{images/verlaufsansicht_mockup}
	\caption{GUI Prototyp -- Verlaufsansicht mit Grenzwert-Referenzlinien}
\end{figure}
\begin{itemize}
	...
	\item Granularität abhängig von Rolle: Mitarbeiter:in sieht Rohdaten; Abteilungsleitung sieht Tagesdurchschnitte für Büroräume.
\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.88\textwidth]{images/gui_prototype/Paperprototype_Manager_Dashboard.png}
	\caption{Papierprototyp des Dashboards der Abteilungsleitung mit Raumübersicht}
\end{figure}

\subsection{Dashboard (Geschäftsführung)}
\begin{itemize}
	\item Firmenweites Dashboard: Aggregierte Werte nach Abteilungen -- keine raumgenauen Daten.
	\item Balkendiagramm: Anzahl Grenzwertverletzungen pro Abteilung (anonymisiert, nicht auf einzelne Räume zurückführbar).
	\item Trendlinie über Wochen.
\end{itemize}

\subsection{Konfigurationsansicht (Hausverwalter)}
\begin{itemize}
...
\subsection{Gerätemanagement (Systemadministrator)}
\begin{itemize}
	\item Übersicht aller Raspberry Pis und Sensorstationen mit Status (Online/Offline/Maintenance).
...
\newpage
\section{Sequenzdiagramm: Normaler Messzyklus mit Grenzwertwarnung}

\textit{Das vollständige Sequenzdiagramm befindet sich als separates PNG im \texttt{/images} Ordner (Datei: \texttt{images/general\_sequence\_diagram.png}).}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{images/general_sequence_diagram}
	\caption{Erster Entwurf eines Sequenzdiagrams des allgemeinen Ablaufs des gesamten Systems}
\end{figure}

Dieser Sequenzdiagramm Entwurf zeigt den typischen Ablauf eines Messzyklus --
von der Messung am Arduino bis zur Anzeige in der Webapp -- inklusive dem
Auslösen und Aufheben einer Grenzwertwarnung.


\textit{Hinweis: Das Sequenzdiagramm wird in V2 um weitere Abläufe ergänzt
	(Verbindungsaufbau, Datenschutz-Abschaltung, Konfigurationsänderung).}
...
\begin{itemize}
    \item Sequenzdiagramm - Grenzwertverletzung des gesamten Systems
    \item Sequenzdiagramm - Belegungs-Mindestgrenze (erledigt, siehe "images/Sequenzdiagramm - Datenschutz Mindestbelegung)
    \item Sequenzdiagramm - Verbindungsaufbau und –konfiguration Arduino / Raspberry / Webapp
\end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{images/Sequenzdiagramm_-_Datenschutz_Mindestbelegung}
    \caption{Sequenzdiagramm -- Belegungs-Mindestgrenze}
\end{figure}
...
\section{Lösungsansatz komplexer Abläufe}


\section{Ausfallssicherheit}

    \newcolumntype{L}{>{\bfseries}p{3.2cm}}

    \subsection{Szenario 1 – Ausfall der Sensorstation}

    \begin{tabularx}{\linewidth}{L X}
    \toprule
    Datenkonsistenz &
    Der Raspberry Pi erkennt den Ausfall über einen BLE-Verbindungs Timeout.
    Da keine neuen Messdaten eintreffen, werden keine fehlerhaften oder unvollständigen
    Einträge in die SQLite-Datenbank geschrieben.
    Bereits gepufferte Daten bleiben erhalten. \\
    \midrule
    Logging &
    Der Raspberry Pi schreibt einen \texttt{ERROR}-Eintrag in sein lokales Logfile
    (\texttt{sensor\_offline}, Sensor-ID, Timestamp). Der Webserver erhält ein entsprechendes
    Event und loggt den Ausfall ebenfalls auf \texttt{ERROR}-Level in beiden Server-Logfiles.
    Die Wiederverbindung wird auf \texttt{INFO}-Level protokolliert. \\
    \midrule
    Wiederaufnahme &
    Sobald die Sensorstation wieder BLE-Advertising mit ihrer bekannten ID sendet,
    baut der Raspberry Pi automatisch die Verbindung wieder auf; kein manuelles
    Eingreifen ist notwendig. \\
    \midrule
    Informationsaus-
    tausch &
    Der Raspberry Pi sendet bei Erkennung des Ausfalls aktiv ein
    \texttt{POST /events \{type: sensor\_offline, sensorId\}} an die Webapp.
    Bei Wiederherstellung folgt ein entsprechendes \texttt{sensor\_online}-Event. \\
    \midrule
    User-Information &
    \begin{minipage}[t]{\linewidth}
        \begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=0pt]
            \item \textbf{Mitarbeiter:innen / Abteilungsleitung:} Hinweis im Dashboard,
            dass die Sensorstation offline ist; Zeitlücke im Verlaufsdiagramm als
            „Daten nicht verfügbar" markiert.
            \item \textbf{Systemadmins:} Gerätestatus in der Verwaltungsansicht zeigt Ausfall; in beiden Logfiles einsehbar.
            \item \textbf{Gebäudeadmins:} Keine direkten Geräteinformationen (Datentrennung),
            aber fehlende Messdaten im Raumverlauf erkennbar.
        \end{itemize}
    \end{minipage} \\
    \bottomrule
    \end{tabularx}
    ...
    \subsection{Szenario 2 – Eingeschränkte BLE-Kommunikation}

    \begin{tabularx}{\linewidth}{L X}
    \toprule
    Datenkonsistenz &
    Die Sensorstation puffert Messwerte lokal im Arduino-Speicher, solange die BLE-Verbindung
    instabil ist. Der Raspberry Pi verwirft unvollständige Pakete und wartet auf eine stabile Übertragung.
    Nur vollständig empfangene und validierte Datenpakete werden in SQLite geschrieben. \\
    \midrule
    Logging &
    Verbindungsprobleme werden auf \texttt{WARN}-Level im Raspberry-Pi-Logfile festgehalten,
    inkl.\ Anzahl verworfener Pakete und Dauer der Instabilität.
    Sobald die Verbindung wiederhergestellt ist, wird dies auf \texttt{INFO}-Level geloggt. \\
    \midrule
    Wiederaufnahme &
    Nach Stabilisierung der BLE-Verbindung überträgt die Sensorstation die gepufferten
    Messwerte kumuliert. Der Raspberry Pi ergänzt Timestamps für jeden Messpunkt
    und verarbeitet die Daten regulär weiter. \\
    \midrule
    Informationsaus-
    tausch &
    Während der Instabilität werden keine unvollständigen Daten an die Webapp weitergeleitet.
    Nach Wiederherstellung erfolgt die Übertragung der aufgelaufenen Daten über ein API Endpoint. \\
    \midrule
    User-Information &
    \begin{minipage}[t]{\linewidth}
        \begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=0pt]
            \item \textbf{Mitarbeiter:innen / Abteilungsleitung:} Lücke im Verlaufsdiagramm
            als „Daten fehlen (Verbindungsproblem)" markiert; keine Echtzeit-Warnung.
            \item \textbf{Systemadmins:} Verbindungsstatus der BLE-Verbindung in der
            Geräteverwaltung; \texttt{WARN}-Einträge in den Logfiles.
        \end{itemize}
    \end{minipage} \\
    \bottomrule
    \end{tabularx}
    ...
    \subsection{Szenario 3 – Unerwarteter Neustart des Raspberry Pi}

    \begin{tabularx}{\linewidth}{L X}
    \toprule
    Datenkonsistenz &
    Alle Datenbankoperationen auf der SQLite-Datenbank werden als ACID-konforme
    Transaktionen ausgeführt. Nach dem Neustart wird die Datenbank auf Integrität geprüft,
    bevor der Betrieb aufgenommen wird. \\
    \midrule
    Logging &
    Der Neustart wird beim ersten Start des Dienstes als \texttt{WARN}-Eintrag protokolliert
    (erkennbar am Fehlen eines regulären \texttt{shutdown}-Eintrags).
    Der Webserver loggt den Reconnect des Raspberry Pi auf \texttt{INFO}-Level. \\
    \midrule
    Wiederaufnahme &
    Der Raspberry-Pi-Dienst startet nach einem Neustart automatisch. Beim Start:
    \begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=0pt]
            \item Laden der Konfiguration aus Config-File.
            \item Abfrage der aktuellen Konfiguration anhand einer API Endpoint.
            \item Automatischer Aufbau der BLE-Verbindungen zu zugeordneten Sensorstationen.
    \end{enumerate} \\
    \midrule
    Informationsaus-
    tausch &
    Nach dem Reconnect sendet der Pi einen initialen Config-Request an den Webserver.
    Der Webserver erkennt den Reconnect und übermittelt ggf.\ ausstehende
    Konfigurationsänderungen. \\
    \midrule
    User-Information &
    \begin{minipage}[t]{\linewidth}
        \begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=0pt]
            \item \textbf{Systemadmins / Gebäudeadmins:} Kurzer Ausfall in den Logs und
            im Gerätestatus sichtbar.
            \item \textbf{Alle Rollen:} Datenlücke im Dashboard entsprechend markiert.
        \end{itemize}
    \end{minipage} \\
    \bottomrule
    \end{tabularx}
    ...
    \subsection{Szenario 4 – Temporärer Ausfall: Raspberry Pi – Backend}

    \begin{tabularx}{\linewidth}{L X}
    \toprule
    Datenkonsistenz &
    Der Raspberry Pi puffert alle eingehenden Messdaten mit Timestamps in SQLite.
    Doppelte Übertragungen werden durch eindeutige Measurement-IDs auf Serverseite
    verhindert. \\
    \midrule
    Logging &
    Fehlgeschlagene Übertragungsversuche werden auf \texttt{ERROR}-Level geloggt.
    Die Wiederherstellung der Verbindung wird auf \texttt{INFO}-Level festgehalten. \\
    \midrule
    Wiederaufnahme &
    Der Raspberry Pi implementiert eine Retry-Queue.
    Grenzwertberechnungen und die Kommunikation mit den Sensorstationen laufen
    während des Ausfalls vollständig autonom weiter; Grenzwerte liegen lokal in SQLite vor,
    sodass keine Webapp-Abhängigkeit besteht.
    Nach Wiederherstellung überträgt der Pi gepufferte Daten chronologisch geordnet. \\
    \midrule
    Informationsaus-
    tausch &
    Nach Wiederherstellung der Verbindung überträgt der Pi die gepufferten Daten
    geordnet an den Webserver.
    Der Server markiert eventuelle Zeitlücken im Datensatz. \\
    \midrule
    User-Information &
    \begin{minipage}[t]{\linewidth}
        \begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=0pt]
            \item \textbf{Mitarbeiter:innen / Abteilungsleitung:} Hinweis auf fehlende Daten
            im betroffenen Zeitraum im Dashboard.
            \item \textbf{Systemadmins:} Verbindungsstatus, Dauer des Ausfalls und
            Retry-Versuche in den Logfiles einsehbar.
            \item \textbf{Lokal (Sensorstation):} LED und Display funktionieren weiterhin
            normal; Grenzwertwarnungen werden lokal angezeigt.
        \end{itemize}
    \end{minipage} \\
    \bottomrule
    \end{tabularx}
    ...
    \subsection{Szenario 5 – Kurzfristiger Ausfall des zentralen Backends}

    \begin{tabularx}{\linewidth}{L X}
    \toprule
    Datenkonsistenz &
    Da alle persistenten Messdaten primär über den Webserver in PostgreSQL gespeichert
    werden, entstehen bei einem Backend-Ausfall keine Schreibkonflikte. Der Raspberry Pi
    puffert in SQLite und überträgt nach Wiederherstellung. Der Webserver stellt beim Start
    die Datenbankverbindung zu PostgreSQL wieder her und verarbeitet aufgelaufene Requests
    aus der Pi-Retry-Queue. \\
    \midrule
    Logging &
    Der Webserver schreibt beim Hochfahren einen \texttt{INFO}-Eintrag.
    Ein externer Health-Check detektiert den Ausfall und schreibt ihn in die Logfiles.
    Alle beim Reconnect eingehenden Verbindungsversuche der Raspberry Pis werden ebenfalls geloggt. \\
    \midrule
    Wiederaufnahme &
    Nach dem Neustart authentifizieren sich die Raspberry Pis automatisch erneut
    und senden ihre gepufferten Daten. Die Webapp ist für alle Nutzer wieder erreichbar. \\
    \midrule
    Informationsaus-
    tausch &
    Während des Ausfalls laufen Sensorstation und Raspberry Pi vollständig autonom weiter;
    Messung, Grenzwertprüfung und lokale Warnung über LED/Display funktionieren
    unabhängig vom Backend. Grenzwert-Events werden in der Pi-Retry-Queue
    zwischengespeichert und nach Wiederherstellung übertragen. \\
    \midrule
    User-Information &
    \begin{minipage}[t]{\linewidth}
        \begin{itemize}[leftmargin=1.2em, itemsep=2pt, topsep=0pt]
            \item \textbf{Mitarbeiter:innen / alle Webapp-Nutzer:} Generische Fehlermeldung
            beim Öffnen der Webapp (\texttt{HTTP 503 Service Unavailable}).
            \item \textbf{Lokal (Sensorstation):} Warnungen über Displays und LEDs
            funktionieren weiterhin uneingeschränkt.
            \item \textbf{Systemadmins:} Ausfall-Zeitraum nach Wiederherstellung in den
            Logfiles einsehbar; Docker-Health-Check-Status protokolliert.
            \item \textbf{Alle Rollen:} Datenlücke im Verlaufsdiagramm als
            „Daten fehlen" gekennzeichnet.
        \end{itemize}
    \end{minipage} \\
    \bottomrule
    \end{tabularx}
