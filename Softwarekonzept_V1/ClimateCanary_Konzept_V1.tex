...
\begin{itemize}
  ...
  \item \textbf{Abteilungsleitung}: Erhält eine Übersicht über alle Räume der eigenen Abteilung in reduzierter Datengranularität. Sieht Abwesenheiten der Mitarbeiter:innen.
  \item \textbf{Höheres Management}: Sieht firmenweite, aggregierte und anonymisierte Übersichten und Trendindikatoren. Kein Zugriff auf raumgenaue Daten.
  \item \textbf{Gebäudeadministration}: Sieht alle Raumklimadaten aller Räume, konfiguriert Grenzwerte und verwaltet Raumklima-Tipps. Kein Zugriff auf Userdaten.
  \item \textbf{Systemadministration}: Verwaltet Benutzer, Gebäudestruktur und Geräte (Raspberry Pis, Sensorstationen). Kein Zugriff auf Messdaten.
\end{itemize}

\subsection{Kernfunktionalitäten}
\begin{itemize}
  \item Automatische Erfassung von Temperatur, Luftfeuchtigkeit und Luftqualität durch Arduino-Sensorstationen.
  \item Übertragung der Messdaten via Bluetooth Low Energy (BLE) an einen raumgebundenen Raspberry Pi.
  \item Lokal am Raspberry Pi: Grenzwertprüfung, datenschutzkonforme Zwischenspeicherung, Weiterleitung an den zentralen Webserver.
  \item Visuelle Rückmeldung direkt an der Sensorstation: LED-Ampel und LCD-Display mit aktuellen Werten, Warnungen und Tipps.
  \item Zentrale Webanwendung mit rollenabhängigen Ansichten, Verlaufsdiagrammen, Grenzwertwarnungen und Konfigurationsoptionen.
  \item Datenschutzkonforme Datenhaltung: Bürodaten werden nur bei Mindestbelegung von 5 Personen persistent gespeichert.
  \item Abwesenheitssystem zur Pflege von Urlaub, Krankheit etc., das automatisch die Datenschutzprüfung beeinflusst.
\end{itemize}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{images/use_cases}
	\caption{Uses cases draft}
	\label{fig:use-cases}
\end{figure}

\textit{Das Use Case Diagramm befindet sich als separates Mermaid-Diagramm im GitLab Wiki (Datei: \texttt{usecase\_diagram.mermaid}).}

\subsection{Akteure}

\begin{itemize}
  \item \textbf{Mitarbeiter:in} -- Basisrolle; hat Zugriff auf eigenes Büro und Allgemeinflächen der Abteilung.
  \item \textbf{Abteilungsleitung} -- Erbt Rechte der Mitarbeiter:in; zusätzlich Abteilungsübersicht.
  \item \textbf{Höheres Management} -- Nur firmenweite, aggregierte Sicht; keine Raumdetails.
  \item \textbf{Gebäudeadministration} -- Vollzugriff auf Raumklimadaten und Konfiguration; kein Zugriff auf Userdaten.
  \item \textbf{Systemadministration} -- Vollzugriff auf Benutzer- und Geräteverwaltung; kein Zugriff auf Messdaten.
  \item \textbf{Raspberry Pi} -- Technischer Akteur; übermittelt Messdaten und empfängt Konfigurationsupdates.
  \item \textbf{Sensorstation (Arduino)} -- Technischer Akteur; misst und zeigt Daten an; empfängt Warnstatus.
\end{itemize}
...
\begin{tabularx}{\textwidth}{lX}
  ...
  \midrule
  Mitarbeiter:in & Raumklima eigenes Büro einsehen (wenn Belegung $\geq$ 5), Allgemeinflächen einsehen, Grenzwarnungen einsehen, Abwesenheit eintragen/verwalten \\
  Abteilungsleitung & + Abteilungsübersicht einsehen, Büros (Tagesdurchschnitte) einsehen, Abwesenheiten Abteilung einsehen, Grenzwertverletzungen Abteilung einsehen \\
  Management & Firmen-Dashboard einsehen, Trendindikatoren einsehen, anonymisierte Verletzungsstatistiken einsehen \\
  Gebäudeadmin & Alle Raumdaten einsehen, Grenzwerte konfigurieren, Raumklima-Tipps verwalten \\
  Systemadmin & Benutzer verwalten, Gebäudestruktur verwalten, Geräte anlegen und zuweisen \\
  \bottomrule
\end{tabularx}
\end{center}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.3\textwidth]{images/class_diagram}
	\caption{Class diagram draft}
	\label{fig:class-diagram}
\end{figure}

\textit{Das vollständige Klassendiagramm befindet sich als separates Mermaid-Diagramm im GitLab Wiki (Datei: \texttt{class\_diagram\_v1.mermaid}).}

\subsection{Klassenbeschreibungen}

\begin{itemize}
  \item \textbf{User} -- Repräsentiert einen Systembenutzer. Attribute: id, name, email, passwordHash, role. Assoziationen: gehört einer Abteilung an, ist einem Büroraum zugeordnet, hat Abwesenheiten.
  \item \textbf{Role} -- Enum: EMPLOYEE, DEPARTMENT\_LEAD, MANAGEMENT, BUILDING\_ADMIN, SYSTEM\_ADMIN.
  \item \textbf{Absence} -- Repräsentiert eine Abwesenheit eines Users. Attribute: id, userId, startDate, endDate, type (URLAUB, KRANKHEIT, KARENZ, etc.).
  \item \textbf{Building} -- Repräsentiert ein Gebäude. Attribute: id, name, address. Enthält Abteilungen.
  \item \textbf{Department} -- Repräsentiert eine Abteilung. Attribute: id, name, buildingId. Enthält Räume und User.
  \item \textbf{Room} -- Repräsentiert einen Raum. Attribute: id, name, type (OFFICE / COMMON\_AREA), departmentId, minOccupancy (default: 5 für Büroräume). Assoziationen: hat einen Raspberry Pi, hat Grenzwerte, hat Messdaten.
  \item \textbf{RaspberryPi} -- Repräsentiert einen Raspberry Pi. Attribute: id (generiert), roomId, ipAddress, status. Assoziationen: hat Sensorstationen, hat Grenzwerte (lokal gespiegelt).
  \item \textbf{SensorStation} -- Repräsentiert eine Sensorstation (Arduino). Attribute: id (generiert, hardcoded auf Arduino), raspberryPiId, status.
  \item \textbf{Measurement} -- Repräsentiert einen Messdatenpunkt. Attribute: id, sensorStationId, timestamp, temperature, humidity, airQuality. Wird nur bei erfüllter Datenschutzbedingung persistent gespeichert.
  \item \textbf{Threshold} -- Repräsentiert Grenzwerte für einen Raum. Attribute: id, roomId, metric (TEMPERATURE / HUMIDITY / AIR\_QUALITY), upperBound, lowerBound (optional).
  \item \textbf{ThresholdViolation} -- Repräsentiert eine aktive oder historische Grenzwertverletzung. Attribute: id, roomId, thresholdId, startTime, endTime (null wenn noch aktiv), metric, value.
  \item \textbf{ClimateHint} -- Repräsentiert einen Raumklima-Tipp. Attribute: id, metric, condition, hintText. Wird bei Grenzwertverletzungen angezeigt.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{images/liniendiagramm_beispiel}
	\caption{Liniendiagramm random Beispiel}
